<template>
  <div class="container">
    ...
    <h1 class="text-2xl font-mono">Favorites</h1>
...
const deleteBook = async (id: number) => {
  ...
  if (resSwal.isConfirmed) {
    $fetch(`/api/books/Favorites/${id}`, { method: "DELETE" })
    .then(async (res) => {
      if (res) {
        grid.value?.fetchData();
        await useToast("success", "Successfully deleting book from favorites collection", "Deleted!")
      }
    })
    .catch(async (err) => {
...
const fetch = {
  request: "/api/books/Favorites/" + profile.id,
  opts: {},
};

const onSaveEdit = () => {
  loadingModel.value.saveButton = true;
  $fetch(`/api/books/Favorites/${profile.id}`, { method: "PUT", body: model.value })
    .then(async (res) => {
